...
func _input(event):
	...
	if event.is_action_pressed("interaction"):
		...
		if (
			fire_alarm_reference != null
			and fire_alarm_reference.useable
			and can_use_alarm
			and self.name == str(multiplayer.get_unique_id())
		):
			sabotage_alarm()
...
@rpc("any_peer", "call_remote") func hide_me(id, isThirdFloor):
...
@rpc("any_peer", "call_remote") func change_alarm_state():
	var fire_alarm_reference = get_node_or_null("../level/fire_alarm")
	fire_alarm_reference.useable = false


@rpc("any_peer", "call_local") func removeBooster():
	var booster = get_node_or_null("../level/booster")
	if booster != null:
...
@rpc("any_peer", "call_local") func remove_obstacle(_obstacle_to_destroy):
